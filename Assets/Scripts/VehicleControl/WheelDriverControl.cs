...
public class WheelDriverControl : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        for (int i = 0; i < wheels.Length; i++)
        {
            var wheel = wheels[i];
            //필요할 때만 바퀴 모양을 만들자.
            if (leftWheelShape != null && wheel.transform.localPosition.x < 0) // 왼쪽에 있는 휠
            {
    ...
    */
    ...
    public void Move(float _acceleration, float _steering, float _brake)
    {
        ...
        foreach (var wheel in wheels)
        {
            ...
            // 뒷바퀴이고 전륜이 아니면 모터토크를 준다.
            if (wheel.transform.localPosition.z < 0 &&
                driveType != DriveType.FrontWheelDrive)
            {
                wheel.motorTorque = torque;
            }
            // 앞바퀴이고 후륜이 아니면 모터토크를 준다.
            if (wheel.transform.localPosition.z > 0 &&
        ...
        if (rb != null)
        {
            float speedUnit = GetSpeedUnit(rb.velocity.magnitude); // magnitude는 벡터의 크기
