...
public static class TrafficHQInspectorEditor
{
    public static void DrawInspector(TrafficHeadquater trafficHeadquater,SerializedObject serializedObject, out bool restructureSystem)
    {
        ...
        InspectorHelper.Toggle("기즈모를 숨길까요?", ref trafficHeadquater.hideGizmos);
        // 화살표 세팅.
        InspectorHelper.DrawArrowTypeSelection(trafficHeadquater);
        InspectorHelper.FloatField("웨이포인트 크기.", ref trafficHeadquater.waypointSize);
        EditorGUILayout.Space();
        // 시스템 설정.
        InspectorHelper.Header("시스템 설정.");
        InspectorHelper.FloatField("구간 감지 최소 거리", ref trafficHeadquater.segDetectThresh);
